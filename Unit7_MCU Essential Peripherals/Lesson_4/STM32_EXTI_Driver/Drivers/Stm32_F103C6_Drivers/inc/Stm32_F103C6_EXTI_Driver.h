...
typedef struct
{
	...
	uint8_t				IRQ_EN ;		//Enable Or Disable the EXTI IRQ (That will Enable the IRQ MASK in EXTI and Also on the NVIC Interrupt)
										//This parameter must be set base on @ref EXTI_IRQ_define

	void (* P_IRQ_CallBack)(void) ;		//Set the function which will called once the IRQ Happen
} EXTI_PinConfig_t;
